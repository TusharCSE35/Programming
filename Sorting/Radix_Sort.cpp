...
void counting_sort(ll A[], ll n, ll pos)
{
	ll ctlen = 10, i;
	ll count[ctlen], Output[n];

	for (i = 0; i < ctlen; i++)count[i] = 0;
	for (i = 0; i < n; i++)count[(A[i] / pos) % 10]++;
	for (i = 1; i < ctlen; i++)count[i] += count[i - 1];
	for (i = n - 1; i >= 0; i--)Output[--count[(A[i] / pos) % 10]] = A[i];
...
void radix_sort(ll A[], ll n)
{
	...
	for (pos = 1; mx / pos > 0; pos *= 10)counting_sort(A, n, pos);
}
